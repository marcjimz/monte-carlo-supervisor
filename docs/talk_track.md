...
- If not cached: routes to `simulation_trigger`, which calls the UC Function to trigger a Databricks Job
...
- Routes through `simulation_checker` → `simulation_trigger` → poll cycle
...
A: First run: typically 2-5 minutes depending on cluster size and num_simulations (default 10,000 trials across 50 Spark partitions). Subsequent identical requests return from cache in under 1 second.
