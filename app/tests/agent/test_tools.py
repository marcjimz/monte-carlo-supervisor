...
def _mock_simulation_service():
    ...
    mock_module.check_simulation = AsyncMock(return_value={"status": "not_found"})
...
class TestCheckSimulation:
    ...
    async def test_returns_json(self, _mock_simulation_service):
        ...
        result = await check_simulation.ainvoke({
            "simulation_type": "encounter_cost_h2",
            "parameters": "{}",
        })
    ...
    async def test_parses_parameters(self, _mock_simulation_service):
        await check_simulation.ainvoke({
            "simulation_type": "cost_comparison",
            "parameters": '{"reduction_pct": 0.08}',
            "num_simulations": 5000,
            "seed": 123,
        })
        _mock_simulation_service.check_simulation.assert_called_once_with(
            "cost_comparison",
            {"reduction_pct": 0.08},
            5000,
            123,
...
class TestTriggerSimulation:
    ...
    async def test_returns_submitted(self, _mock_simulation_service):
        ...
        result = await trigger_simulation.ainvoke({
            "simulation_type": "encounter_cost_h2",
        })
        parsed = json.loads(result)
